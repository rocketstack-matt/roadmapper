...
const storeConfirmToken = async (token, keyHash) => {
  await redis.set(`confirm:${token}`, keyHash, { ex: REGISTRATION_TTL });
};

const lookupConfirmToken = async (token) => {
  const keyHash = await redis.get(`confirm:${token}`);
  return keyHash;
};
...
const confirmRegistration = async (token) => {
  ...
  return { success: true, owner: keyData.owner, repo: keyData.repo };
};
